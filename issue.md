...
| F-76 | `n8n.md` section 7 signed `draw.isoDate`, `draw.mainNumbers` and `draw.bonusNumber`, fields section 3.4's parser never writes; `JSON.stringify` drops undefined keys, so the body would have been `{}` and every rebuild refused | `nextStep/n8n.md` section 7 step 4 and the same snippet in `nextStep/lottodraw.md` - now `targetKey`, `numbers`, `bonus` |
