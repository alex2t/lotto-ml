...
- **A caption states what the analyzer counted, checked against the analyzer.** The scenario table
  was captioned "came up again in the next draw" while it counts windows (F-70).
...
- **Motion is reduced to a cross-fade, not to nothing.** `prefers-reduced-motion` turns the four
  ...
  arrives rather than blinking into place (web.md 5.2). A Statistics card that grows open simply
  opens at once.
