...
This runs the 158 real tests across the sixteen real test files, re-derives train/serve parity from the
data, validates every generated ticket, and prints the validation metrics with the 2 SE noise floor.
It exits non-zero if anything fails. The sixteen files, and what each one guards, are listed in
