...
This runs the 167 real tests across the eighteen real test files, re-derives train/serve parity from the
data, validates every generated ticket, and prints the validation metrics with the 2 SE noise floor.
It exits non-zero if anything fails. The eighteen files, and what each one guards, are listed in
