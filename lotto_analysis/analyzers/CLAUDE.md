...
- **This layer has no train/validation split, deliberately.** It computes over every draw, which is
  ...
  draw, and the next draw has no future to leak from. Training never reads these categories -
  `walk_forward.py` derives `category` itself, point-in-time. The only split that exists is
